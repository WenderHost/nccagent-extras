...
function custom_breadcrumbs( $atts ) {
    ...
    if ( is_archive() && ! is_tax() && ! is_category() && ! is_tag() ) {
      ...
      $debug['switch']['type'] = 'page';

        // Standard page
        if( $post->post_parent ){
          $debug['switch']['post_parent'] = $post->post_parent;
    ...
    if( current_user_can( 'activate_plugins' ) )
      $html.= '<script type="text/javascript">console.log(`🔔 Supercrumbs Debug: `, ' . json_encode( $debug ) . ');</script>';
...
function get_subnav( $post_parent_id, $post_type = 'page' ){
  ...
  $carriers_and_products_parents = ['plans','carriers','products','product-finder'];
  if( $children || 'carrier' == $post_type || in_array( $current_slug, $carriers_and_products_parents ) ){
    if(
      'carrier' == $post_type
      || 'product' == $post_type
      || in_array( $parent_slug, $carriers_and_products_parents )
      || in_array( $current_slug, $carriers_and_products_parents )
    ){
