    ...
    $.get( getUrl, { per_page: 1, orderby: 'rand', state: termId } )
     .done( function(response){
      if( 0 === response.length ){
        ...
        $('#ncc-staff').html(`<div class="marketer row"><div class="col team-member-photo"><div class="stretchy-wrapper"><div class="photo" style="background-image: url(${marketer.photo});"></div></div></div><div class="col team-member-details"><h3 class="team-member-name"><a href="${marketer.permalink}">${marketer.name}</a></h3><h4 class="team-member-title">${marketer.title} • ${marketer.phone} • <a href="mailto:${marketer.email}">${marketer.email}</a></h4><div class="team-member-read-more"><a href="${marketer.permalink}">Read testimonials from ${marketer.firstname}'s agents</a></div></div></div>`);
      }
    });
  }
