...
(function($){
  ...
  const dtOptions = {
    ...
    columns: [
      ...
      {
        data: function(d){
          return `<h4 class="row-title"><a href="${d.product.url}">${d.carrier.name} ${d.product.alt_name}</a></h4><a href="#" class="details-link">See plan details right here <i class="fas fa-chevron-down"></i></a>`;
        },
        orderable: false
      }
    ],
  ...
  $(`#${wpvars.table_id}`).on('click', 'a.details-link', function(e){
    ...
    if( row.child.isShown() ){
      ...
      var d = row.data();
      row.child( $(`<tr><td class="child-details" colspan="4"><h5>State Availability</h5><div class="states">${d.states}</div><h5>Plan Information</h5>${d.description}<h5>Request a Product Kit for ${d.carrier.name} ${d.product.alt_name}</h5><p>We’ll email you a kit with brochures, commissions, and rates specifically for ${selectedState} or another state of your choice.</p><p><a class="elementor-button" href="${d.kit_request_url}">Request a Kit</a></p><p><a href="${d.product.url}">View this information as a web page.</a></p></td></tr>`) ).show();
      $(this).addClass('shown')
      $(this).children('i.fas').toggleClass('fa-chevron-down fa-chevron-up')
      $(tr).addClass('shown')
    }
