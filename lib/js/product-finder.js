...
(function($){
  ...
  const dtOptions = {
    ...
    columns: [
      ...
      {
        data: function(d){
          return `<h4 class="row-title"><a href="#" class="details-link">${d.carrier.name} ${d.product.alt_name}</a></h4>`;
        },
        orderable: false
      }
    ],
        ...
        if( colIdx === (totalColumns - 4) ){
          ...
          column.data().unique().sort().each( function ( data, index ) {
            ...
            if( ( index + 1 ) === this.length && 2 === colIdx ){
              ...
              // Sort/Alphabetize our other options:
  ...
  $(`#${wpvars.table_id}`).on('click', 'a.details-link', function(e){
    ...
    if( row.child.isShown() ){
      ...
      var d = row.data();
      var statesReviewDate = ( typeof d.states_review_date !== 'undefined' && '' !== d.states_review_date )? `<p class="review-date">Current as of ${d.states_review_date}</p>` : '' ;
      var descReviewDate = ( typeof d.desc_review_date !== 'undefined' && '' !== d.desc_review_date )? `<p class="review-date">Current as of ${d.desc_review_date}</p>` : '' ;
      var issueAges = ( '' !== d.lower_issue_age && '' !== d.upper_issue_age )? `<p>Issue ages ${d.lower_issue_age}&ndash;${d.upper_issue_age}.</p>` : '' ;
      var extraLinks = `<ul><li><a href="">${d.carrier.name} Contracting Online</a></li><li><a href="${d.carrier.url}">${d.carrier.name} Main Page with All Products</a></li><li><a href="${d.product.url}">View this plan information as a web page</a></li></ul>`;
      var medicareNote = ( -1 < d.product.name.indexOf('Medicare') )? `<p>Some information may vary by state. <a href="../tools/medicare-quote-engine/">See state-specific information and rates</a>.</p>` : '';

      row.child( $(`<tr><td class="child-details" colspan="4"><p><a href="${d.product.url}">View this information as a web page.</a></p><h5>State Availability in addition to ${selectedState}</h5>${statesReviewDate}<div class="states">${d.states}</div><h5>Plan Information</h5>${descReviewDate}${issueAges}${d.description}${medicareNote}${extraLinks}<div class="kit-request"><h5>Request a Product Kit for ${d.carrier.name} ${d.product.alt_name}</h5><p>We’ll email you a kit with brochures, commissions, and rates specifically for ${selectedState} or your choice of other states.</p><p><a class="elementor-button" href="${d.kit_request_url}">Request a Kit</a></p></div><!-- .kit-request --></td></tr>`) ).show();
      $(this).addClass('shown')
      $(this).children('i.fas').toggleClass('fa-chevron-down fa-chevron-up')
      $(tr).addClass('shown')
    }
