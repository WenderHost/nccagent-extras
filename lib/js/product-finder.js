  ...
  $(`#${wpvars.table_id}`).on('click', 'a.details-link', function(e){
    ...
    if( row.child.isShown() ){
      ...
      if( typeof selectedState !== 'undefined' ){
        productKitVerbiage = `We’ll email you a kit with brochures, commissions, and rates specifically for ${selectedState} or your choice of other states.`;
      } else {
        productKitVerbiage = `We’ll email you a kit with brochures, commissions, and rates for your choice of states.`;
      }

      row.child( $(`<tr><td class="child-details" colspan="4"><p><a href="${d.product.url}">View this information as a web page.</a></p><h3>${stateAvailabilityHeading}</h3>${statesReviewDate}<div class="states">${d.states}</div><h3>Plan Information</h3>${descReviewDate}${issueAges}${d.description}${medicareNote}${extraLinks}<div class="kit-request"><h3>Request a Product Kit for ${d.carrier.name} ${d.product.alt_name}</h3><p>${productKitVerbiage}</p><p><a class="elementor-button" href="${d.kit_request_url}">Request a Kit</a></p></div><!-- .kit-request --></td></tr>`) ).show();
      $(this).addClass('shown')
      $(this).children('i.fas').toggleClass('fa-chevron-down fa-chevron-up')
      $(tr).addClass('shown')
    }
