...
(function($){
  ...
  const dtOptions = {
    ...
    columns: [
      ...
      {
        data: function(d){
          return `<h4 class="row-title"><a href="${d.product.url}">${d.carrier.name} ${d.product.alt_name}</a></h4><div class="states">${d.states}</div><a href="#" class="details-link"> Product Details</a>`;
        },
        orderable: false
      }
    ],
  ...
  $(`#${wpvars.table_id}`).on('click', 'a#reset-form', function(e){
    e.preventDefault()
    resetSelects()
  })
