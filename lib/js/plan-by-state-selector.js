(function($){
  ...
  const dtState = window.localStorage.getItem(`DataTables_datatable_/${wpvars.product_finder_slug}/`);
  let dtStateObj = {};
  if( dtState === null ){
  ...
  $('#plan-by-state-selector').on('change','#states',function(e){
    ...
    console.log(`👉 Plan by State Selector:`);
    console.table({state: state, product: wpvars.product});

    // Set the `State`
    dtStateObj.columns[1].search.search = state;
    // Set the `Product`
    dtStateObj.columns[2].search.search = wpvars.product;
    // Clear any saved Carriers
    dtStateObj.columns[3].search.search = '';
    // Update the DataTables saved state:
    window.localStorage.setItem( `DataTables_datatable_/${wpvars.product_finder_slug}/`, JSON.stringify(dtStateObj) );
  });
