    ...
    $.get( getUrl, { per_page: 1, orderby: 'rand', state: termId } )
     .done( function(response){
      if( 0 === response.length ){
        ...
        $('#ncc-staff').html(`<div class="marketer"><div class="photo" style="background-image: url(${marketer.photo});"></div><h3>${marketer.name}</h3><h4>${marketer.title} • ${marketer.phone} • <a href="mailto:${marketer.email}">${marketer.email}</a></h4>${marketer.bio}</div>`);
      }
    });
  }
