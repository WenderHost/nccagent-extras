...
console.log('API: ', wpvars.planFinderApi );
...
(function($){
  ...
  const dtOptions = {
    ...
    columns: [
      ...
      {
        data: function(d){
          return `<h4 class="row-title"><a href="${d.product.url}">${d.carrier.name} ${d.product.alt_name}</a></h4><div class="states">${d.states}</div><a href="#" class="details-link"> Plan Details</a>`;
        },
        orderable: false
      }
    ],
  ...
  $(`#${wpvars.table_id}`).on('click', 'a.details-link', function(e){
    ...
    if( row.child.isShown() ){
      ...
      var d = row.data();
      row.child( $(`<tr><td class="child-details" colspan="4"><!--<h5>${d.carrier.name} ${d.product.alt_name}</h5>-->${d.description}</td></tr>`) ).show();
      $(this).addClass('shown');
    }
  });
    ...
    if( empty ){
      ...
      if( filtered ){
        ...
        if( '' !== wpvars.helpGraphic ){
          ...
          empty.html('Select one or more options to find plans.');
        }
      }
    }
  });
    ...
    $.get( getUrl, { per_page: 1, orderby: 'rand', state: termId } )
     .done( function(response){
      if( 0 === response.length ){
        ...
        $('#ncc-staff').html(`<div class="marketer row"><div class="col"><div class="photo team-member-photo" style="background-image: url(${marketer.photo});"></div></div><div class="col team-member-details"><h3 class="team-member-name"><a href="${marketer.permalink}">${marketer.name}</a></h3><h4 class="team-member-title">${marketer.title} • ${marketer.phone} • <a href="mailto:${marketer.email}">${marketer.email}</a></h4><div class="team-member-read-more"><a href="${marketer.permalink}">Read testimonials from ${marketer.name}'s agents &rarr;</a></div></div></div>`);
      }
    });
  }
