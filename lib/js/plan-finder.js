    ...
    $.get( getUrl, { per_page: 1, orderby: 'rand', state: termId } )
     .done( function(response){
      if( 0 === response.length ){
        ...
        $('#ncc-staff').html(`<div class="marketer row"><div class="col-sm-2"><div class="photo" style="background-image: url(${marketer.photo});"></div></div><div class="col-sm-10"><h3>${marketer.name}</h3><h4>${marketer.title} • ${marketer.phone} • <a href="mailto:${marketer.email}">${marketer.email}</a></h4>${marketer.bio}</div></div>`);
      }
    });
  }
